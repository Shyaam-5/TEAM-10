# Gemini OCR endpoint for image-to-text
import base64
...
try:
    from docling.document_converter import DocumentConverter, InputDocument as _DoclingInputDocument  # type: ignore
    _DoclingSource = None  # type: ignore
    DocumentInput = None  # type: ignore
except Exception as _e_docling_primary:
    try:
        # Fallback for alternative package name/layout
        from docling_core.document_converter import DocumentConverter, InputDocument as _DoclingInputDocument  # type: ignore
        _DoclingSource = None  # type: ignore
        DocumentInput = None  # type: ignore
    except Exception as _e_docling_alt:
        DocumentConverter = None  # type: ignore
        _DoclingSource = None  # type: ignore
        _DoclingInputDocument = None  # type: ignore
        try:
            _docling_import_error = str(_e_docling_alt) or str(_e_docling_primary)
        except Exception:
            _docling_import_error = "Unknown Docling import error"
...
def ocr_gemini():
        ...
        "contents": [
            {
                "parts": [
                    ...
                    {"inline_data": {"mime_type": image.mimetype, "data": img_b64}}
                ]
            }
        ]
    }
    ...
    if resp.status_code == 200:
        ...
        return jsonify({'success': False, 'error': 'Gemini API error'}), 500
...
# ---------------------------
# Global Docling setup (warm-up at startup)
# ---------------------------
from docling.document_converter import DocumentConverter
import tempfile, time
...
@app.route('/api/ocr/docling', methods=['POST'])
def ocr_docling():
    """Extract text from uploaded PDF using Docling"""
    file = request.files.get('file')
    if not file or not file.filename.endswith('.pdf'):
        return jsonify({'success': False, 'error': 'No valid PDF uploaded'}), 400
    ...
    try:
        ...
        app.logger.info(f"[Docling] Starting conversion for {file.filename}")

        # Run Docling conversion (no options param in your version)
        result = converter.convert(tmp_path)
        doc = result.document

        elapsed = time.time() - start_time
        app.logger.info(f"[Docling] Conversion finished in {elapsed:.2f} seconds")

        # Export extracted text
        extracted_text = doc.export_to_markdown() or ""
        if not extracted_text.strip():
            return jsonify({'success': False, 'error': 'No text extracted from PDF'}), 200

        return jsonify({'success': True, 'text': extracted_text})

    except Exception as e:
        app.logger.error(f"[Docling OCR error] {e}", exc_info=True)
        return jsonify({'success': False, 'error': f'Docling error: {str(e)}'}), 500
# Chatbot routes
# ---------------------------
